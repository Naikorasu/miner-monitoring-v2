let axios,moment,fs,Table;_f34‍.x([["default",()=>_f34‍.o]]);_f34‍.w("axios",[["default",["axios"],function(v){axios=v}]]);_f34‍.w("moment",[["default",["moment"],function(v){moment=v}]]);_f34‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_f34‍.w("tty-table",[["default",["Table"],function(v){Table=v}]]);
	...
	getMarketPrice() {
		...
		axios
			...
			.then((response) => {
				...
				_f34‍.g.console.log("BTCUSDT:", btcPrice);
	...
	setData(key, value) {
		...
		try {
			...
			fs.writeFile("data.json", JSON.stringify(save), function (err, data) {
				if (err) {
					return _f34‍.g.console.log("ERROR:", err);
				}
				//console.log(data);
			});
		} catch (err) {
			_f34‍.g.console.log("ERROR:", err);
		}
		// require("dotenv").config();
		...
		try {
			if (rigDataSet != undefined || rigDataSet != "undefined") {
				...
				try {
					...
					_f34‍.g.console.log(t1);

					const t2 = Table(h2, summaryData, o1).render();
					_f34‍.g.console.log(t2);
				} catch (err) {
					_f34‍.g.console.log("ERROR:", err);
				}
